...
export const initiateSignupOTP = async ({ name, email, password }) => {
...
export const verifySignupAndCreate = async ({ otp, session }) => {
  const { signupOTP, signupEmail, signupName, signupPassword, signupOTPExpiry } = session;

  if (!signupOTP || !signupEmail)          throw new Error('Session expired. Please signup again.');
  if (Date.now() > signupOTPExpiry)        throw new Error('OTP has expired. Please request a new one.');
  ...
  const user   = new User({ name: signupName, email: signupEmail, password: hashed });
  await user.save();
  return user;
};
...
export const verifyResetOTP = ({ otp, session }) => {
  ...
  if (Date.now() > resetOTPExpiry)        throw new Error('OTP has expired. Please request a new one.');
...
export const resendOTPToSession = async (session) => {
  const email   = session.resetEmail || session.signupEmail;
  const context = session.resetEmail ? 'forgot-password' : 'signup';
  if (!email) throw new Error('Session expired. Please start again.');

  const otp    = generateOTP();
  const expiry = Date.now() + 5 * 60 * 1000;
  console.log(`[OTP] Resend OTP (${context}) for ${email} → ${otp}`);
  await sendOTP(email, otp, 'Resend OTP - Velmora Chroné');
  return { otp, expiry, isReset: !!session.resetEmail };
};